...
class ConsulDiscovery(ServiceDiscovery):
    ...
    def discover(self, service_name: str) -> HostAndPort:
        ...
        if service_name not in registrations:
            if service_name in self.discovery_threads.keys():
                self.discovery_threads.get(service_name).stop()

            thread = threading.Timer(
                config("CONSUL_DISCOVERY_INTERVAL", 60),
                self.do_discover,
            )
            self.discovery_threads.update({service_name: thread})
            thread.start()

        return (
            random.choice(registrations)
            if registrations
            else self.do_discover(service_name)
        )

    # open a thread and do_discover on an interval, might need to modify do_discover
    ...
    def do_discover(self, service_name: str) -> HostAndPort:
        ...
        if service_name not in self.services:
            ...
            return None
        healthy_services = self.consul_client.health.service(
            service=service_name, passing=True
        )
        registrations = [
            HostAndPort(entry["Service"]["Address"], entry["Service"]["Port"])
...
def configure_views(app):
    ...
    def consul_catalog(service_name: str, service_discovery: ServiceDiscovery):
        ...
        return (
            ...
            if registration
            else f"No registration for {service_name}"
        )
